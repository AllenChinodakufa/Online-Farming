...
if (isset($_GET['delete'])) {
    $sql = "DELETE FROM `tbl_crops` WHERE `tbl_crops`.`CROP_ID` = ".$_GET['delete'];
        ...
        <div class="dash-aside">
            ...
            <div class="section-table">
                <table class="table-responsive">
                    ...
                    <tbody>
                        <?php
                        $sql = "SELECT * FROM `tbl_crops` WHERE `USER_ID`= '".get_userid($user)."'";
                        $sqlresult = mysqli_query($conn, $sql);
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="ion-buttons">
                                            <a href="#" data-delete="<?php echo $row["CROP_ID"]; ?>" class="ion-button danger">
                                                <i class="ion-ios-trash-outline"></i>
                                            </a>
                                            <a href="#" data-edit="<?php echo $row["CROP_ID"]; ?>"  data-name="<?php echo $row["NAME"]; ?>" data-dimensions="<?php echo $row["DIMENSIONS"]; ?>" data-category="<?php echo $row["CATEGORY"]; ?>" class="ion-button primary">
                                                <i class="ion-edit"></i>
                                            </a>
                                        </div>
                                    </td>
...
<script>
    ...
    const actionSheetCropsEdit = function (header, message, itemid, name, category, dimensions) {
        ...
        <form class="alert-wrapper">
            ...
            <div class="alert-body">
                ...
                <div class="form-group">
                    ...
                    <input id="dimensions" type="number" placeholder="Item Dimensions.." value="${dimensions}" required>
                    <span class="form-response">
                        <span class="success">
                            <p>Looks good</p>
                        </span>
    ...
    const actionSheetCropsTo = function (header, message) {
        ...
        <form class="alert-wrapper">
            ...
            <div class="alert-body">
                ...
                <div class="form-group">
                    <label for="name">name:</label>
                    <input id="name" type="text" placeholder="Your Item.." required>
                    <span class="form-response">
                        <span class="success">
                            <p>Looks good</p>
                        </span>
                ...
                <div class="form-group">
                    <label for="dimensions">dimesnions:</label>
                    <input id="dimensions" type="number" placeholder="Item dimensions.." required>
                    <span class="form-response">
                        <span class="success">
                            <p>Looks good</p>
                        </span>
    ...
    const addcrops = function (formfield) {
        ...
        $(`${formfield} a[data-action="submit"]`).on("click", function () {
            if (check === true) {
                ...
                ajax.open("POST", "Config/cropsconfig.php");

                ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");

                ajax.send("name=" + $(`${formfield} input#name`).val() + "&category=" + $(`${formfield} input#category`).val() + "&dimensions=" + $(`${formfield} input#dimensions`).val());
            }
            return check;
        });
    ...
    const addEdit = function (formfield, itemid) {
        ...
        $(`${formfield} a[data-action="submit"]`).on("click", function () {
            if (check === true) {
                ...
                ajax.open("POST", "Config/cropsedit.php");

                ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");

                ajax.send("id=" + `${itemid}` + "&name=" + $(`${formfield} input#name`).val() + "&category=" + $(`${formfield} input#category`).val() + "&dimensions=" + $(`${formfield} input#dimensions`).val());
            }
            return check;
        });
    ...
    tblDelete.click(function(){
        alertTo("crops", `Are you sure you would like to delete item #${$(this).attr('data-delete')}`, `crops.php?delete=${$(this).attr('data-delete')}`);
    });

    tblEdit.click(function(){
        actionSheetCropsEdit("crops", `Are you sure you would like to edit item #${$(this).attr('data-edit')}`, `${$(this).attr('data-edit')}`, `${$(this).attr('data-name')}`, `${$(this).attr('data-category')}`, `${$(this).attr('data-dimensions')}`);
    });

    tblAdd.click(function(){
        actionSheetCropsTo("crops", `Add a new item to your crops`);
    });
</script>
