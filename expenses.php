...
?>
<body>
    <div class="dash" style="background-image: url('assets/img/main (7).jpg');">
        ...
        <div class="dash-aside">
            ...
            <div class="section-table">
                <table class="table-responsive">
                    ...
                    <tbody>
                        <?php
                        $sql = "SELECT * FROM `tbl_inventory` WHERE `TYPE` = 'EXPENSE' AND `USER_ID`= '".get_userid($user)."'";
                        $sqlresult = mysqli_query($conn, $sql);
...
<script>
    ...
    const addStock = function (formfield) {
        ...
        $(`${formfield} a[data-action="submit"]`).on("click", function () {
            if (check === true) {
                ...
                ajax.open("POST", "Config/expenseconfig.php");
    ...
    const addEdit = function (formfield, itemid) {
        ...
        $(`${formfield} a[data-action="submit"]`).on("click", function () {
            if (check === true) {
                ...
                ajax.open("POST", "Config/expenseedit.php");
    ...
    tblDelete.click(function(){
        alertTo("Expenses", `Are you sure you would like to delete item #${$(this).attr('data-delete')}`, `expenses.php?delete=${$(this).attr('data-delete')}`);
    });

    tblEdit.click(function(){
        actionSheetStockAEdit("Expenses", `Are you sure you would like to edit item #${$(this).attr('data-edit')}`, `${$(this).attr('data-edit')}`, `${$(this).attr('data-item')}`, `${$(this).attr('data-category')}`, `${$(this).attr('data-cost')}`);
    });

    tblAdd.click(function(){
        actionSheetStockTo("Expenses", `Add a new item to your stock`);
    });
</script>
